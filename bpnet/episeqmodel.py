...
class EpiSeqModel:
    ...
    def __init__(self,
        ...
        self.epi_names = epi_names

        seqmodel = SeqModel.from_mdir(self.model_dirs[0])
        new_model = Model(inputs=seqmodel.model.input,
                      outputs=seqmodel.model.get_layer(seqmodel.bottleneck_name.split('/')[0]).output)
        for i in range(len(new_model.layers)):
            new_model.layers[i].trainable = False
        inp = kl.Input(shape=(self.seqlen, 4), name='seq')
        all_inps = [inp]
        cinp = new_model(inp)

        for midx,model_dir in enumerate(self.model_dirs[1:]):
            seqmodel = SeqModel.from_mdir(model_dir)
            new_model = Model(inputs=seqmodel.model.input,
                      outputs=seqmodel.model.get_layer(seqmodel.bottleneck_name.split('/')[0]).output)
            for i in range(len(new_model.layers)):
                new_model.layers[i].trainable = False
            inp = kl.Input(shape=(self.seqlen, 1), name=f'epi_tracks/{self.tasks[0]}/{self.epi_names[midx]}')
            all_inps.append(inp)
            c = new_model(inp)
            cinp = kl.concatenate([cinp,c])
        ...
        self.model = Model(bias_inputs + all_inps, outputs)
    ...
    def _contrib_grad_fn(self, x, name, preact_only=True):
        ...
        for name, target_tensor in zip(intp_names, target_tensors):
            ...
            self.contrib_fns["grad/" + name] = K.function(input_tensor,
                                                          K.gradients(target_tensor, input_tensor))
    ...
    def contrib_score(self, x, name, method='grad', batch_size=512, preact_only=False):
        ...
        if batch_size is None:
            return fn(input_to_list(input_names, x))[0],fn(input_to_list(input_names, x))[1]
        else:
            return numpy_collate_concat([fn(input_to_list(input_names, batch))[0]
                   ...
                   numpy_collate_concat([fn(input_to_list(input_names, batch))[1]
                                          for batch in nested_numpy_minibatch(x, batch_size=batch_size)])
...
config.external_configurable(EpiSeqModel)
