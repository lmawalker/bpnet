...
class TaskSpec(RelatedConfigMixin):
    ...
    tracks = StrSequenceField(str, required=False)
...
class AddedTrack(RelatedLoadSaveMixin):
...
class DataSpec(RelatedLoadSaveMixin):
    ...
    # Paths to secondary input
    added_track_dict = related.MappingField(AddedTrack,"name",required=False)
    ...
    #directories of added epigenetic models
    epi_model_dirs = related.MappingField(AddedTrack, "name", required=False)

    # # Set of peaks to consider
    # peaks = related.StringField(required=True)

    # path to dna sequence model
    dna_model_dir = related.StringField(required=False)

    # path to dna bias model
    bias_model_dir = related.StringField(required=False)

    # Original path to the file
    path = related.StringField(required=False)

    def abspath(self):
        print(self.added_track_dict)
        for added_track in self.added_track_dict.values():
            added_track.path = os.path.abspath(added_track.path)
        ...
        return DataSpec(
            ...
            added_track_dict=self.added_track_dict,
            path=self.path)
    ...
    def touch_all_files(self, verbose=True):
        ...
        # Let vmtouch see additional input tracks
        for key,added_track_object in self.added_track_dict.items():
            touch_file(os.path.abspath(added_track_object.path),verbose)
