...
class MidcnnSeqModel:
    ...
    def __init__(self,
        ...
        seqmodel = SeqModel.from_mdir(self.model_dirs[1])
        new_model = Model(inputs=seqmodel.model.input,
                      outputs=seqmodel.model.get_layer(seqmodel.bottleneck_name.split('/')[0]).output)
        for i in range(len(new_model.layers)):
            new_model.layers[i].trainable = False
        inp = kl.Input(shape=(self.seqlen, 4), name=f'bias/{self.tasks[0]}/bias_fasta')
        all_inps.append(inp)
        c = new_model(inp)
...
config.external_configurable(MidcnnSeqModel)
