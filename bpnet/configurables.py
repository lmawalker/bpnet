...
@gin.configurable
def determine_num_channels(exclude_dna: bool, num_added_tracks_in_dataspec, channels_to_remove):
    # Determine the number of channels to expect
    num_channels = 4
    if exclude_dna:
        num_channels = 0
    # add additional from dataspec
    num_channels += num_added_tracks_in_dataspec
    num_channels -= len(channels_to_remove)
    return num_channels
