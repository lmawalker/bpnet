...
from bpnet.EpiBPNet import EpiBPNetSeqModel
...
def bpnet_export_bw(model_dir,
    ...
    gin_config = read_json(f'{model_dir}/config.gin.json')
    try:
        do_epi = gin_config['do_epi']
    except KeyError:
        print(f"do_epi not found in gin {model_dir}/config.gin.json\nSetting to False")
        do_epi = False
    if do_epi:
        ds = DataSpec.load(os.path.join(model_dir, 'dataspec.yml'))
        abs_ds = DataSpec.load(ds.path)
        bp = EpiBPNetSeqModel.from_mdir(model_dir,abs_ds)
    else:
