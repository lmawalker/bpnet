...
from bpnet.biasfactseqmodel import BiasFactSeqModel
from bpnet.episeqmodel import EpiSeqModel
from bpnet.midcnnseqmodel import MidcnnSeqModel
...
@arg("bias_fasta",
     help='True/False use bias encoded fasta sequence')
def list_contrib(model_dir,bias_fasta,do_epi,model_type):
    ...
    if do_epi:
        seqmodel = EpiSeqModel.from_mdir(model_dir)
    elif bias_fasta:
        if model_type == 'midcnn':
            seqmodel = MidcnnSeqModel.from_mdir(model_dir)
        else:
            seqmodel = BiasFactSeqModel.from_mdir(model_dir)
    else:
...
def bpnet_contrib(model_dir,
    ...
    exclude_dna = config['exclude_dna']
    try:
        special_encode = config['special_encode']
    except KeyError:
        print(f"special_encode not found in gin {model_dir}/config.gin.json\nSetting to \'none\'")
        special_encode = 'none'
    try:
        bias_fasta = config['bias_fasta']
    except KeyError:
        print(f"bias_fasta not found in gin {model_dir}/config.gin.json\nSetting to False")
        bias_fasta = False
    try:
        bias = config['bias']
    except KeyError:
        if '_biasfact_specialencodeca2' in model_dir:
            print(f"bias not found in gin {model_dir}/config.gin.json but ca2 found in model name\nSetting to \'ca2\'")
            bias = 'ca2'
        elif '_biasfact_specialencodeca4' in model_dir:
            print(f"bias not found in gin {model_dir}/config.gin.json but ca4 found in model name\nSetting to \'ca4\'")
            bias = 'ca4'
        else:
            print(f"bias not found in gin {model_dir}/config.gin.json\nSetting to \'ca4\'")
            bias = 'ca4'
    try:
        do_epi = config['do_epi']
    except KeyError:
        print(f"do_epi not found in gin {model_dir}/config.gin.json\nSetting to False")
        do_epi = False
    try:
        model_type = config['model_type']
    except KeyError:
        print(f"model_type not found in gin {model_dir}/config.gin.json\nSetting to \'seqmodel\'")
        model_type = 'seqmodel'
    ...
    for i, batch in enumerate(tqdm(dl_valid.batch_iter(batch_size=batch_size,
        ...
        if do_epi:
            for name in ['rep1/counts/pre-act','rep1/profile/wn']:
                print(name)
                m_inputs = {'seq': batch['inputs']['seq']}
                for track_name in [x for x in batch['inputs'] if 'epi_tracks' in x]:
                    m_inputs[track_name] = batch['inputs'][track_name]

                hyp_contrib = seqmodel.contrib_score(m_inputs,
                                                     name=name,
                                                     method=method,
                                                     batch_size=None)  # don't second-batch
                print(hyp_contrib[0].shape)
                print(hyp_contrib[1].shape)
                print(hyp_contrib[0])
                print(hyp_contrib[1])
                batch[f"/hyp_contrib/{name}/seq"] = hyp_contrib[0][:, trim_start:trim_end]
                batch[f"/hyp_contrib/{name}/epi"] = hyp_contrib[1][:, trim_start:trim_end]
        else:
            for name in intp_names:
                if bias_fasta:
                    hyp_contrib = seqmodel.contrib_score({'seq': batch['inputs']['seq'],
                                                          'bias/rep1/bias_fasta': batch['inputs']['bias/rep1/bias_fasta']},
                                                         name=name,
                                                         method=method,
                                                         batch_size=None)  # don't second-batch
                else:
        ...
        if bias_fasta:
            batch['inputs']['bias/rep1/bias_fasta'] = batch['inputs']['bias/rep1/bias_fasta'][:, trim_start:trim_end]
        if do_epi:
            for track_name in [x for x in batch['inputs'] if 'epi_tracks' in x]:
                batch['inputs'][track_name] = batch['inputs'][track_name][:, trim_start:trim_end]
