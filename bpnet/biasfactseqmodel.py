"""Bias Factorzied sequence model
"""
...
class BiasFactSeqModel:
    ...
    def __init__(self,
        ...
        self.heads = heads
        self.seqlen = seqlen
        self.bias_model_path = bias_model_path
        self.n_bias_tracks = n_bias_tracks

        if input_shape is None:
            input_shape = (self.seqlen, channel_shape)
        ...
        for task in tasks:
            # get bias model outs
            wo_bias_model = SeqModel.from_mdir(self.bias_model_path)
            #wo_bias_model.input_name = f'bias/rep1/bias_fasta'
            num_layers = len(wo_bias_model.model.layers)
            for i in range(num_layers):
                wo_bias_model.model.layers[i].trainable = False
            fbinp = kl.Input(shape=(self.seqlen, self.n_bias_tracks), name=f'bias/{task}/bias_fasta')
            wo_bias_outs = wo_bias_model.model(fbinp)
            for outidx,head in enumerate(heads):
                head = head.copy()
                self.all_heads[task].append(head)
                out, bias_input = head(bottleneck,wo_bias_outs[outidx], task)
                outputs.append(out)
        ...
        self.model = Model([inp] + [fbinp] + bias_inputs, outputs)
...
config.external_configurable(BiasFactSeqModel)
