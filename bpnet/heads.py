...
class ScalarHead(BaseHeadWBias):
    ...
    def __call__(self, inp, task):
        print('\n\n')
        print("ScalarHead input shape:", K.int_shape(inp))
        print('\n\n')
        o = self.net(inp)
        print('\n\n')
        print("ScalarHead after net shape:", K.int_shape(o))
        print('\n\n')
    ...
    def get_preact_tensor(self, graph=None):
        ...
        try:
            ...
            return graph.get_tensor_by_name('dense_1/BiasAdd:0')
    ...
    def intp_tensors(self, preact_only=False, graph=None):
        ...
        try:
            ...
            if self.activation is None:
                ...
                # have any specific meaning when
                # we don't use any activation function
                return {"pre-act": graph.get_tensor_by_name('dense_1/BiasAdd:0')}

            if preact_only:
                return {"pre-act": graph.get_tensor_by_name('dense_1/BiasAdd:0')}
            else:
                return {"pre-act": graph.get_tensor_by_name('dense_1/BiasAdd:0'),
                        "output": graph.get_tensor_by_name('dense_1/BiasAdd:0')}
...
class ProfileHead(BaseHeadWBias):
    ...
    def get_preact_tensor(self, graph=None):
        ...
        try:
            ...
            return graph.get_tensor_by_name('reshape_2/Reshape:0')
    ...
    def intp_tensors(self, preact_only=False, graph=None):
        ...
        try:
            ...
            preact = graph.get_tensor_by_name('reshape_2/Reshape:0')
            postact = graph.get_tensor_by_name('reshape_2/Reshape:0')
...
class BiasFactBaseHead:
    ...
    @abc.abstractmethod
    def __call__(self, inp, wo_bias_out, task):
        """Useful for writing together the model
        Returns the output tensor
        """
        raise NotImplementedError
...
class BiasFactBaseHeadWBias(BiasFactBaseHead):
...
@gin.configurable
class BiasFactScalarHead(BiasFactBaseHeadWBias):
    ...
    def __call__(self, bottleneck_inp, wo_bias_out, task):
        o = self.net(bottleneck_inp)

        bias_inputs = []

        o = kl.Add()([wo_bias_out, o])
...
@gin.configurable
class BiasFactProfileHead(BiasFactBaseHeadWBias):
    """Deals with the case where the output are multiple tracks of
    total shape (L, C) (L = sequence length, C = number of channels)
