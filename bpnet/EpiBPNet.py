...
class EpiBPNetSeqModel:
    """EpiBPNet based on EpiSeqModel
    """

    def __init__(self, seqmodel, abs_ds, fasta_file=None):
        self.seqmodel = seqmodel
        self.tasks = self.seqmodel.tasks
        self.fasta_file = fasta_file
        self.abs_ds = abs_ds
        assert isinstance(self.seqmodel, EpiSeqModel)
        # TODO - add some sanity checks (profile head available etc)

    @classmethod
    def from_mdir(cls, model_dir, abs_ds):
        from bpnet.episeqmodel import EpiSeqModel
        ...
        if os.path.exists(ds_path):
            ...
            fasta_file = None
        return cls(EpiSeqModel.from_mdir(model_dir), abs_ds, fasta_file=fasta_file)
    ...
    def contrib_score_all(self, seq, method='deeplift', aggregate_strand=True, batch_size=512,
                          pred_summaries=['profile/wn/seq', 'counts/pre-act/seq','profile/wn/epi', 'counts/pre-act/epi']):
    ...
    def _get_old_contrib_score_name(self, s):
        ...
        s2s = {'profile/wn/seq': 'profile.seq', 'counts/pre-act/seq': 'count.seq',
               'profile/wn/epi': 'profile.epi', 'counts/pre-act/epi': 'count.epi'}
        return s2s[s]
    ...
    def predict_all(self, seq, contrib_method='grad', batch_size=512, pred_summaries=['profile/wn/seq', 'counts/pre-act/seq','profile/wn/epi', 'counts/pre-act/epi']):
    ...
    def predict_regions(self, regions,
        ...
        m_inputs = [seq]
        for track_name in self.abs_ds.epi_model_dirs.keys():
            for task in self.tasks:
                epi_extractor = BigwigExtractor(
                    DataSpec.load(f'{self.abs_ds.epi_model_dirs[track_name].path}/dataspec.yml').task_specs[task].tracks[0])
                m_inputs.append(epi_extractor(regions).reshape(epi_extractor(regions).shape[0],-1,1))

        preds = self.predict_all(m_inputs, contrib_method, batch_size, pred_summaries=pred_summaries)
    ...
    def export_bw(self,
        ...
        output_feats = ['preds', 'contrib.profile.seq', 'contrib.counts.seq',
                        'contrib.profile.epi', 'contrib.counts.epi']
